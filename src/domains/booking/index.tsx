...
export default function BookingView() {
    ...
    useEffect(() => {
        ...
        getUserDetails(user.uid).then((data: UserDetails) => {
            const ud = data;

            if (ud.type.includes("host")) {
                populateVenueDetails(ud.venues).then((venues: Venue[]) => {
                    setUserDetails(ud);
                    setOrganiser(ud);
                    setVenues(venues);
                    setSelectedVenue(venues[0]);
                });
            } else {setUserDetails(ud);}
    ...
    const genres = ["Acoustic", "Alternative Rock", "Bluegrass", "Ceilidh", "Children's Music", "Christian", "Classic Rock", "Classical", "Country", "Disco", "Folk", "Funk", "Gospel Music", "Grunge", "Heavy Metal",  "Hip Hop", "House", "Indie Rock", "Jazz", "Latin", "Opera", "Pop", "Punk Rock", "R&B",  "Rap", "Reggae", "Soca", "World Music", "Other"];
    ...
    const handleNewMinFee: ChangeEventHandler =
        (e: ChangeEvent<HTMLInputElement>) => setMinFee(parseInt(e.currentTarget.value));

    const handleNewMaxFee: ChangeEventHandler =
        (e: ChangeEvent<HTMLInputElement>) => setMaxFee(parseInt(e.currentTarget.value));
        ...
        <Container className="m-0 p-0">
            <Row>
                ...
                <div className="col-sm-11">
                    <Header title={`Booking - ${newBooking ? "New Booking" : bookingID}`}/>
                        ...
                        <Container>
                            <Row>
                                <Col>
                                    ...
                                    <InputGroup>
                                        {organiser && (<div id="event-organiser-group">
                                            <Form.Label id="organiser-name">{organiser.name}</Form.Label>
                                            <Button onClick={() => navigate(`/profile/${organiser.uid}`)}>View Profile</Button>
                                        </div>)}
                                    </InputGroup>
                                </Col>
                                <Col>
                                        ...
                                        <InputGroup className="mb-3">
                                                ...
                                                >
                                                    {eventTypes.map((eventType, index) => (
                                                        <Dropdown.Item key={index} onClick={() => eventTypeChanged(eventType)}>{eventType}</Dropdown.Item>
                                                    ))}
                                                </DropdownButton>

                                                {(eventType && eventType === "Other") && (<Form.Control type="text" value={otherEventType} onChange={e => setOtherEventType(e.target.value)} placeholder="Event type"/>)}
                                        </InputGroup>
                                    </Form.Group>
                                </Col>
                                <Col>
                                    <Form.Group>
                                        ...
                                        <InputGroup className="mb-3">
                                                <DropdownButton variant="outline-secondary" title={genre ? genre : "Select Genre"} id="input-group-dropdown-1" >
                                                    {genres.map((g, index) => (
                                                        <Dropdown.Item key={index} onClick={() => genreChanged(g)}>{g}</Dropdown.Item>
                                                    ))}
                                                </DropdownButton>
                                    ...
                                    <Container>
                                        <Row>
                                            ...
                                            <Col>
                                                <InputGroup className="mb-3">
                                                    ...
                                                    <FormControl aria-label="Amount" type="number" value={minFee} onChange={handleNewMinFee} />
                                        ...
                                        <Row>
                                            ...
                                            <Col>
                                                <InputGroup className="mb-3">
                                                    <InputGroup.Text>£</InputGroup.Text>
                                                    <FormControl aria-label="Amount" type="number" value={maxFee} onChange={handleNewMaxFee} />
                                                    <InputGroup.Text>.00</InputGroup.Text>
                                                </InputGroup>
                                            </Col>
                                        </Row>
                            ...
                            <Row>
                                <Form.Group>
                                    ...
                                    <Form.Control type="date" name="bookingDate" placeholder="Booking date" value={date} onChange={e => setDate(e.target.value)}/>
                                    ...
                                    <Container style={{marginTop:"2vh"}}>
                                        <Row>
                                            ...
                                            <Col>
                                                <Form.Control type="time" name="bookingStartTime" placeholder="Start Time" value={startTime} onChange={e => setStartTime(e.target.value)}/>
                                            </Col>
                                        </Row>
                                        <Row style={{marginTop:"1vh"}}>
                                            ...
                                            <Col>
                                                <Form.Control type="time" name="bookingEndTime" placeholder="End Time" value={endTime} onChange={e => setEndTime(e.target.value)}/>
                                            </Col>
                                        </Row>
                                    </Container>
                            ...
                            <Row>
                                <Form.Group>
                                    <Form.Label className="label"><b>Event Publicity</b></Form.Label>
                                    
                                    <InputGroup className="mb-1">
                                            <DropdownButton variant="outline-secondary" title={eventPrivacy}>
                                                <Dropdown.Item onClick={() => {setEventPrivacy("Public")}}>Public</Dropdown.Item>
                                                <Dropdown.Item onClick={() => {setEventPrivacy("Private")}}>Private</Dropdown.Item>
                                            </DropdownButton>
                                    </InputGroup>

                                    <p className="small-print">A public event is free or ticketed, and open to anyone. A private event has a specific guest list and will not be shown on the events calendar.</p>
                                </Form.Group>
                            </Row>
