...
import { Button, Container, Form, Row, InputGroup, Col, Carousel } from "react-bootstrap";
...
export default function VenueViewer() {
    ...
    useEffect(() => {
        ...
        getUserDetails(user.uid).then((ud: UserDetails) => setUserDetails(ud));
        ...
        <Container className="global-container">
            <Row>
                ...
                <Col style={{padding: 0}}>
                    ...
                    {newVenue && userDetails && venue && <VenueCreator createVenue={createVenue} user={userDetails} venue={venue}/>}
                    
                    {!newVenue && venue && !isEditing && <VenueInfo user={userDetails!} venue={venue} navigate={navigate}  createVenue={createVenue}/>}
...
function VenueCreator(props: VenueCreaterProps) {
    ...
    useEffect(() => {
        if (plainFiles.length) {
            uploadVenuePhoto(props.venue.uid, plainFiles[0]).then((url: string | null) => {
                    ...
                    <Col>
                        ...
                        <InputGroup>
                            {props.venue.organiser && (<div id="event-organiser-group">
                                <Form.Label id="organiser-name">{props.venue.organiser}</Form.Label>
                                <Button onClick={() => props.navigate(`/profile/${props.venue.organiser}`)}>View Profile</Button>
                            </div>)}
                        </InputGroup>
